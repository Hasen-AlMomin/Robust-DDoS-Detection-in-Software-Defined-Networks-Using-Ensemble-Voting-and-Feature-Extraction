...
#Defintions..  (SSIP . Speed Of source IP) =  SIGS (source IP Generation Speed), (SFE. Speed Of flow Entry) FER =  FLOW ENtry Rate, (PFTR.Pair Of flow Table) FSR = FLOW Success Ratio, (stdp.Standar Deviation Of Packet) PLV= Packet Load Variance, (stdb.Standard Deviation of Byte) BLV=Byte Load Variance
    ...
    def flow_stats_reply_handler(self, ev):
        ...
        if flags == 0:
            ...
            if APP_TYPE == 1 and get_iteration(dpid) == 1:
                ...
                # Convert the value in 'result' to an integer
                result = int(result)
                if result == 1:
                #if  1 in result:
